...
BasePage.prototype = {
    load: function (action, datas) {
        ...
        /*tploader.get(this.tpl.tplKey, function (tpl) {
            _this.rend(tpl, data);
            _this.invoke('load', [datas]);
        ...
        /*tploader.load(this.templateId, function (tpl) {
            _this.rend(tpl, data);
            _this.invoke('load', [datas]);
    ...
    callAction: function(action, datas){
        ...
        var _this = this;
            ...
            callback = function(){},
                ...
                /*console.log('_this.actionRended:', _this.actionRended);
                if(!_this.actionRended){
                    _this.showAction.call(_this, action, $.extend(datas, tplDatas));
                }else{
                    _this.actionRended = false;
                }*/
                
                callback(datas);
                // if(typeof actionMethod === 'function'){
                //     _this.showAction.call(_this, action, $.extend(datas, tplDatas));
                // }else if(typeof actionMethod === 'object' && actionMethod instanceof Page.Action){
                //     var factory = actionMethod.factory,
                //         callback = actionMethod.callback;
                //     factory.call(_this, action, $.extend(datas, tplDatas));
                //     setTimeout(function(){
                //         callback.call(_this);
                //     }, 0);
                // }
    ...
    /*showAction: function(action, datas){
        var subTplId = this.id + '_' + action,
            subTpl = this.getSubTemplate(subTplId);
        if(!subTpl || !this.dom){
            return;
        }
        var container = this.dom.find('.spa-page-action:eq(0)');
        if(container.size() === 0){
            container = this.dom;
        }
        container.html(this.template(subTpl, datas));
        this.url = location.hash;
    },*/
    ...
    rend: function (tpl, data) {
        ...
        if(pagecache && listPageCache){
            ...
            //     this.scrollTop = sessionStorage.scrollTop;
            // }
    ...
    bind: function (elm, evt, handler) {
        ...
        //$(elm).bind(evt, handler);
    },
    unbind: function (elm, evt, handler) {
        delete this.eventHandlers[handler];
        //$(elm).unbind(evt, handler);
        //console.log('unbind:', elm, evt);
    ...
    /*getTemplate: function (id, callback) {
        tploader.load(id, callback);
    },*/
    /*getSubTemplate: function (id) {
        var tpl = tploader.get(id);
        if (!tpl) {
            return null;
        }
        return tpl.content;
    ...
    setTitle: function (title) {
        ...
        /*var $iframe = $('<iframe src="http://weixin.vipstatic.com/uploadfiles/spa/bigBrand/images/holder.png"></iframe>').on('load', function() {
            setTimeout(function() {
